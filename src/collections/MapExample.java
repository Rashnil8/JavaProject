...
public class MapExample {
	...
	public static void main(String[] args) {
		...
		basket.put("English", 85); //English- (Key), 85- (Value)
		basket.put("Hindi", 75);
		basket.put("Marathi", 92);
		basket.put("Math", 88);
		
		System.out.println("The map is " + basket);
		
		basket.put("Politics", 74); //New item add with new item
		System.out.println("The map after change is " + basket);
		
		basket.remove("Hindi"); //delete item
		System.out.println("The map after delete is " + basket);
		
		for(String item:basket.keySet()) {
			System.out.println("Subject name is " + item); //sub call item
			System.out.println("Mark " + basket.get(item));
		}
